...
dstp="$(echo "3600.0*21.0*365.25/$datmstp/$N_TIMESTEPS" | bc -l)"
...
# set 'an average/3D fields' frequency [NOTE: a '+1' in effect disables this feature]
let stp=$RUNLENGTH*$N_TIMESTEPS+1
...
# set goldstein year length in days
let stp=$N_TIMESTEPS/2
echo ea_8=$stp >> $CONFIGPATH/$CONFIGNAME
echo go_8=$stp >> $CONFIGPATH/$CONFIGNAME
echo gs_8=$stp >> $CONFIGPATH/$CONFIGNAME
echo el_8=$stp >> $CONFIGPATH/$CONFIGNAME
